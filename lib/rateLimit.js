...
// Warned once per instance rather than per request, so a misconfiguration
// shows up in the logs without drowning them.
...
//
// Every failure path is logged. Failing open silently is what let the limiter
// sit there doing nothing while looking installed — an unenforced limit has to
// be noisy, or it is worse than no limit at all because it is trusted.
  ...
  try {
    if (!process.env.SUPABASE_SERVICE_KEY) {
      if (!warnedMissingKey) {
        ...
        console.error(
          "[rateLimit] SUPABASE_SERVICE_KEY is not set in this environment — " +
          "rate limiting is DISABLED and every request is being allowed through."
        );
      }
    ...
    if (error) {
      // Reached Postgres but the call was rejected — typically a key that is
      // present but wrong/expired, or a missing grant on the function.
      console.error(
        `[rateLimit] bump_rate_limit failed (${error.code || "no code"}): ${error.message} ` +
    ...
    console.error("[rateLimit] threw, allowing request through:", err?.message || err);
    return { allowed: true, remaining: limit };
  }
}
